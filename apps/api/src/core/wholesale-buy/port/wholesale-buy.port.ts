...
// management net-position view compares buy against sell in. It is not the unit an operator
...
// what lets the UI tell an operator how long their order stays editable. Default midnight;
// override to match the actual cron schedule.
export function autoConfirmHour(): number {
    return resolveAutoConfirmHour(process.env.WHOLESALE_BUY_AUTO_CONFIRM_HOUR)
}
