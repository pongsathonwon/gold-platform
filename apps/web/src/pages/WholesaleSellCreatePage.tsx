...
const RESET_ON_CHANGE: Record<string, (keyof SellValues)[]> = {
  productTypeId: ["purityId", "weight"],
  purityId: ["weight"],
};

export function WholesaleSellCreatePage() {
  const navigate = useNavigate();
  const { showToast } = useToast();
  const { data: suppliersRes } = useSuppliers();
  const { data: productTypesRes } = useProductTypes();
  const createTransaction = useCreateWholesaleSell();
  ...
  const matchingRule = (v: SellValues) => purityRules.find((p) => p.purityId === v.purityId);

  function handleChange(name: keyof SellValues, value: string) {
    setValue(name, value);
  ...
  const fields: FieldConfig<SellValues>[] = [
    ...
    },
    {
      ...
      // two separately-typed prices could disagree, a derived one cannot. For a 99.9% deal the
  ...
  function handleSubmit(e: FormEvent) {
    ...
    createTransaction.mutate(parsed.data, {
      onSuccess: () => {
        showToast("สร้างรายการขายส่งแล้ว");
        navigate("/wholesale-sell");
      },
